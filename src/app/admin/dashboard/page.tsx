...
export default function AdminDashboard() {
  ...
  useEffect(()=>{
    if(isSignedIn && user){
      ...
      if(user &&user.primaryEmailAddress?.emailAddress !=='amnaadnan011@gmail.com'){
        router.replace("/")
      }
    }
      ...
      <nav className="bg-[#2A254B] text-white p-4 shadow-lg flex justify-between">
        ...
        <div className="flex space-x-4">
          {["All", "pending", "dispatch", "success"].map((status) => (
            <button
              key={status}
              className={`px-4 py-2 rounded-lg transition-all ${filter === status ? "bg-white text-[#2A254B] font-bold" : "text-white"
                }`}
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredOrders.map((order) => (
                <React.Fragment key={order._id}>
                  <tr
                    className="cursor-pointer hover:bg-white transition-all "
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <button
                        ...
                        className="bg-[#2A254B] text-white px-3 py-1 rounded hover:bg-red-700 transition"
                      >
                        Delete
                      </button>
                    </td>
